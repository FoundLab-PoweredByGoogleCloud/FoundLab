# **Uma Proposta de Infraestrutura de Confiança Auditável para o BTG Pactual**
...
A plataforma FoundLab (codinome "Umbrella") não é uma ferramenta RegTech incremental. É uma re-arquitetura de primeiros princípios que resolve o dilema central do setor: o conflito between a inovação ágil, a segurança de dados (LGPD) e a retenção imutável de provas (SOX, BACEN).
...
* **Implementação Técnica:**  
  ...
  2. **Encadeamento de Prova (Hash-Chaining):** Cada evento na trilha de auditoria é "selado" criptograficamente, contendo o hash do evento anterior (previousChainHash). Qualquer adulteração quebra a cadeia.  
  3. **Ledger Imutável (WORM):** A trilha de auditoria (Veritas\_Audit\_Trail) é persistida no **Google BigQuery**, configurado como **Write-Once, Read-Many** (WORM).  
...
* **Implementação Técnica:**  
  1. **Fallback Auditado:** Uma arquitetura Multi-Engine (Google Gemini \+ NVIDIA NIMs) garante que, se um motor de IA primário falhar, um secundário assume. O evento de falha (ENGINE\_FAILURE) e o de *fallback* são registados no Protocolo Veritas.  
  ...
  3. **IA Flywheel (Hot Loop):** Este é o ciclo de aprendizagem. Quando um humano corrige uma decisão (um evento HUMAN\_OVERRIDE), essa correção é registada no Veritas e usada como *ground truth* para retreinar e aprimorar os modelos de IA.  
...
graph TD  
    ...
    subgraph "Protocolo Veritas"  
        B \-- "Gera Evento 'HUMAN\_OVERRIDE'" \--\> C((Ledger Imutável));  
...
graph TD  
    subgraph "Perímetro Soberano do BTG (Projeto GCP do Cliente)"  
        direction LR  
          
        subgraph "Trindade da Confiança"  
            CMEK\["Pilar 1: CMEK/HSM  
(Controle de Chaves)"\]  
            VPC\_SC\["Pilar 2: VPC Service Controls  
(Controle de Perímetro)"\]  
            PSC\["Pilar 3: Private Service Connect  
(Controle de Rede)"\]  
        end

        subgraph "Infraestrutura do BTG"  
            CoreBanking\[Sistemas Legados (Core Banking)\]  
            WORM\_Ledger\["BigQuery WORM  
(Custódia do BTG)"\]  
        end

        subgraph "Aplicação FoundLab (na VPC do BTG)"  
            ACL\[Anti-Corruption Layer (ACL)\]  
            Umbrella\[Plataforma Umbrella (Cloud Run)\]  
        end

        CoreBanking \-- "1. Fato Legado" \--\> ACL  
        ACL \-- "2. Traduz" \--\> Umbrella  
        Umbrella \-- "3. Prova WORM" \--\> WORM\_Ledger  
          
        WORM\_Ledger \-- "Criptografado com" \--\> CMEK  
        ACL \-- "Controlado por" \--\> VPC\_SC  
        Umbrella \-- "Controlado por" \--\> VPC\_SC  
        WORM\_Ledger \-- "Controlado por" \--\> VPC\_SC  
          
        subgraph "Serviços Externos (Ex: FoundLab)"  
             Servicos\[Serviços de Suporte FoundLab\]  
        end

        Servicos \-- "Conexão Segura (PSC)" \--\> Umbrella  
    end
